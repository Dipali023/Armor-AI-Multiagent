...
// ========================= AI RESPONSES =========================
const AI_RESPONSES = {
  shelter:{kw:['shelter','camp','refuge','stay','roof','nearest shelter','आश्रय','शरण','camp','नज़दीकी','आश्रय कहाँ'],
  ...
  road:{kw:['road','route','drive','highway','blocked','safe road','travel','سڑک','सड़क','सुरक्षित','रास्ता','मार्ग','routes','safe routes','सड़क सुरक्षित','कौन सी सड़क'],
  ...
  medical:{kw:['medical','hospital','doctor','ambulance','medicine','sick','injured','hurt','चिकित्सा','अस्पताल','डॉक्टर','दवा','एंबुलेंस','सहायता चाहिए'],
  ...
  rescue:{kw:['rescue','trapped','stranded','stuck','help','emergency','flooding','sinking','rooftop','बचाओ','फंसा','मदद','बचाएं','बचाव','फँसा हुआ हूँ'],
  ...
  heatwave:{kw:['heatwave','heat','hot','temperature','heat stroke','sun','गर्मी','तपिश'],
  ...
  ndrf:{kw:['ndrf','ndma','government force','rescue team','एनडीआरएफ','एनडीआरएफ से संपर्क'],
  ...
  supplies:{kw:['supply','supplies','kit','bag','pack','food','water','what to carry','essential','सामग्री','राशन','supply kit','emergency supply'],
  ...
  donate:{kw:['donate','donation','contribute','pm cares','fund','money','दान','सहायता','how to donate','donate money'],
  ...
  cpr:{kw:['cpr','cardiac','heart attack','unconscious','not breathing','सीपीआर','प्राथमिक','cpr guide'],
...
// ========================= NEW: ORCHESTRATION EXPAND =========================
function expandOrchestration() {
  const overlay = document.getElementById('orch-expanded-overlay');
  const expLog = document.getElementById('orch-exp-log');
  // Mirror current agent-log contents into the expanded view
  const srcLog = document.getElementById('agent-log');
  if (expLog && srcLog) expLog.innerHTML = srcLog.innerHTML;
  overlay.classList.add('active');
}
function closeExpandedOrch() {
  document.getElementById('orch-expanded-overlay').classList.remove('active');
}

// ========================= NEW: ARMOR AI EXPAND =========================
let expandedAIMessages = [];

function expandArmorAI() {
  const overlay = document.getElementById('ai-expanded-overlay');
  overlay.classList.add('active');
  const expMessages = document.getElementById('ai-exp-messages');
  if (expMessages && expandedAIMessages.length === 0) {
    // Seed with welcome message
    addExpandedMsg('🤖 **Armor AI — Expanded Command Mode**\n\nWelcome to the full-screen AI Command Interface. All 4 agents are active and ready.\n\nType your question or use the quick command buttons →', false);
  }
  document.getElementById('ai-exp-input')?.focus();
}
function closeExpandedAI() {
  document.getElementById('ai-expanded-overlay').classList.remove('active');
}

function addExpandedMsg(text, isUser) {
  const msgs = document.getElementById('ai-exp-messages');
  if (!msgs) return;
  const div = document.createElement('div');
  div.className = 'message ' + (isUser ? 'user' : 'ai');
  const time = new Date().toLocaleTimeString('en', {hour:'2-digit', minute:'2-digit'});
  ...
  expandedAIMessages.push({ text, isUser });
}
...
function sendExpandedMessage() {
  ...
  const text = input?.value.trim();
  if (!text) return;
  addExpandedMsg(text, true);
  input.value = '';
  logAgentAction('TriageAgent', `[Expanded AI] Received query: "${text}"`, 'triage');
  setTimeout(() => {
    const response = getAIResponse(text);
    addExpandedMsg(response, false);
  }, 700 + Math.random() * 500);
}

function sendExpandedQR(text) {
  const input = document.getElementById('ai-exp-input');
...
document.addEventListener('keydown', (e) => {
  ...
  // Enter in expanded AI input
  const expInput = document.getElementById('ai-exp-input');
  if (e.key === 'Enter' && document.activeElement === expInput) {
    sendExpandedMessage();
  }
});

// ========================= NEW: AGENT LOG COUNT + NODE STATUS =========================
let agentLogCount = 1;

// Patch logAgentAction to also update node chips & log count
const _origLogAgentAction = logAgentAction;
// Override with extended version (keep original behavior + add UI updates)
function logAgentAction(agentName, message, type = 'system') {
  const logEl = document.getElementById('agent-log');
  if (!logEl) return;
  const time = new Date().toLocaleTimeString('en', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
  ...
  // Mirror to expanded log if open
  const expLog = document.getElementById('orch-exp-log');
  if (expLog) {
    const expDiv = document.createElement('div');
    expDiv.className = div.className;
    expDiv.textContent = div.textContent;
    expLog.appendChild(expDiv);
    expLog.scrollTop = expLog.scrollHeight;
  }

  // Update log count
  agentLogCount++;
  const countEl = document.getElementById('orch-log-count');
  if (countEl) countEl.textContent = `${agentLogCount} events`;

  // Flash the agent node chip
  const chipName = agentName.toLowerCase().includes('system') ? 'SystemAgent'
    : agentName.toLowerCase().includes('triage') ? 'TriageAgent'
    : agentName.toLowerCase().includes('geo') ? 'GeoAgent'
    : agentName.toLowerCase().includes('match') ? 'MatchAgent'
    : agentName.toLowerCase().includes('dispatch') ? 'DispatchAgent'
    : agentName.toLowerCase().includes('safety') ? 'SafetyAgent' : null;
  
  if (chipName) {
    const chip = document.querySelector(`.agent-node-chip[data-agent="${chipName}"]`);
    const statusDot = chip?.querySelector('.anc-status');
    if (statusDot) {
      const wasidle = statusDot.classList.contains('idle');
      statusDot.className = 'anc-status busy';
      setTimeout(() => {
        statusDot.className = wasidle ? 'anc-status online' : 'anc-status online';
      }, 1800);
    }
    // Also flash the AI status strip badges
    const agentKey = agentName.toLowerCase().replace('agent','');
    const asnBadge = document.querySelector(`#asn-${agentKey} .asn-badge`);
    if (asnBadge) {
      const prevClass = asnBadge.className;
      const prevText = asnBadge.textContent;
      asnBadge.className = 'asn-badge busy';
      asnBadge.textContent = 'ACTIVE';
      setTimeout(() => {
        asnBadge.className = prevClass.includes('active') ? 'asn-badge active' : 'asn-badge standby';
        asnBadge.textContent = prevClass.includes('active') ? 'ACTIVE' : 'STANDBY';
      }, 2000);
    }
  }
}
...
function clearAgentLog() {
  const logEl = document.getElementById('agent-log');
  if (logEl) logEl.innerHTML = '<div class="log-line system">[SystemAgent] Log cleared. Systems still online.</div>';
  agentLogCount = 1;
  const countEl = document.getElementById('orch-log-count');
  if (countEl) countEl.textContent = '1 events';
}

// ========================= AUTO FULLSCREEN ON LOAD =========================
window.addEventListener('load', () => {
  // Attempt fullscreen automatically (works only on user gesture in some browsers;
  // placed here so it fires on initial page load click or direct open)
  const tryFS = () => {
    if (!document.fullscreenElement) {
  ...
  // Immediately try (works in some browsers on load)
  document.documentElement.requestFullscreen().catch(() => {
